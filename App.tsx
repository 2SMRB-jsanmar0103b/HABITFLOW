...
const MISSIONS: Mission[] = [
  ...
  { id: 'premium2', title: "Leyenda", description: "Consigue una racha de 7 días", rewardType: 'icon', rewardValue: 'minimal', requiredPlan: 'premium', goal: 7, type: 'streak_record' },
              ...
              <div className="space-y-8">
                ...
                {selectedData.archived && selectedData.archived.length > 0 && (
                  <div className="space-y-4 pt-4 border-t border-black/5 dark:border-white/5">
                    <h4 className="text-[9px] font-bold uppercase tracking-widest text-emerald-500">Graduados</h4>
                    <div className="space-y-2">
                      ...
                      {user.subscriptionPlan === 'premium' && (
                        <div className="mb-10 pb-10 border-b border-white/5 space-y-12">
                          ...
                          <div className="space-y-10">
                             ...
                             <div className="space-y-4">
                                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.3em] pl-2">{curT.iconSet}</p>
                                <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
                                   {CUSTOM_ICONS.map(i => (
                                     <button 
                                       key={i.name} 
                                       onClick={() => handleUpdateCustomization('iconSet', i.name.toLowerCase())}
                                       className={`aspect-square rounded-2xl bg-black/40 flex items-center justify-center transition-all hover:bg-brand-lime hover:text-black shadow-lg ${user.customization?.iconSet === i.name.toLowerCase() ? 'bg-brand-lime text-black ring-2 ring-brand-lime ring-offset-4 ring-offset-black' : 'text-gray-400'}`}
                                     >
                                       <i.icon size={24} />
                                     </button>
                                   ))}
                 ...
                 <button onClick={() => handleUpdateCustomization('cursor', 'crosshair')} className={`px-5 py-3 rounded-xl text-[10px] font-bold uppercase transition-all shadow-md ${user.customization?.cursor === 'crosshair' ? 'bg-brand-lime text-black' : 'bg-black/40 text-gray-500'}`}>{curT.customTarget}</button>
                      ...
                      {user.subscriptionPlan !== 'premium' && (
                        <div className="mb-10 p-8 bg-brand-lime/10 border border-brand-lime/20 rounded-3xl flex flex-col items-center text-center gap-4">
                           ...
                           <p className="text-xs text-gray-500 font-medium">Personaliza colores de botones, iconos globales y cursores con el plan Premium.</p>
                        </div>
                      )}
